<#
.SYNOPSIS
    Resets OpenCode to a fresh-test state and writes the Anthropic custom-provider
...
.DESCRIPTION
    ...
    NOTE: OpenCode's support for custom Anthropic-compatible endpoints is
    experimental. This script writes the configuration, but whether the
    resulting setup can communicate through AgentRedactor depends on the
    installed @ai-sdk/anthropic package and OpenCode version.
...
.NOTES
    ...
    Usage:
      .\scripts\setup-opencode-test-anthropic-claude-haiku.ps1
      .\scripts\setup-opencode-test-anthropic-claude-haiku.ps1 -Reinstall
      .\scripts\setup-opencode-test-anthropic-claude-haiku.ps1 -BaseUrl "http://127.0.0.1:8081/"
#>
...
param(
    ...
    [string]$ApiKey = "test",
    [string]$ModelId = "anthropic/claude-haiku-4.5",
    [string]$ModelName = "",
...
Write-Host ""
Write-Host "=== OpenCode Test Setup (Anthropic) ===" -ForegroundColor Cyan
Write-Host ""
...
Install-ProviderPackage -PackageName "@ai-sdk/anthropic" -WhatIf:$WhatIfPreference
...
Write-Host ""
Write-Host "=== OpenCode test setup complete (Anthropic) ===" -ForegroundColor Green
