...
.NOTES
    ...
    Usage:
      .\scripts\setup-hermes-agent-test-anthropic-claude-haiku.ps1
      .\scripts\setup-hermes-agent-test-anthropic-claude-haiku.ps1 -Reinstall
      .\scripts\setup-hermes-agent-test-anthropic-claude-haiku.ps1 -BaseUrl "http://127.0.0.1:8081/" -ApiKey "my-key"
#>
...
param(
    ...
    [string]$ApiMode = "anthropic_messages",
...
function Invoke-HermesModelSetup {
    ...
    $settings = @(
        ...
        # Hermes defaults to a very large max_tokens for Anthropic models, which
        # exceeds the test OpenRouter key's per-request credit budget for Haiku.
        # Cap the requested output tokens so the request stays within the limit.
